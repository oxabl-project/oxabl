...
- `oxabl_lint`: 5 rules — `undefined-symbol` (LINT0001), `unused-variable` (LINT0002; a variable passed as a write-back `OUTPUT`/`INPUT-OUTPUT` `RUN` argument counts as used), `unknown-table-or-field` (LINT0003, live under a loaded schema), `type-mismatch-assignment` (LINT0004), `block-var-used-outside` (LINT0005, INFO: a block-defined variable read outside its block and never assigned outside it — may still hold its default value); per-rule severity via `[workspace.lint]`
