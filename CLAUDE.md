...
- Preprocessor include resolution: an unresolvable `{include}` emits a **loud** `PREPROC007` warning ("symbols it declares cannot be checked") that the CLI always surfaces (stderr for `check`, stderr + the `preproc` envelope section for `analyze`) — one honest diagnostic at the true cause instead of a flood of downstream `undefined-symbol` false positives. Known gap: a **nested** unresolvable include is silent in every shared-pipeline client, because `expand_source` filters loud preproc diagnostics to root-origin only; the conformance walk still prints it
