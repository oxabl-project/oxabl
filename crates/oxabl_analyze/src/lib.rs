...
pub use collect::{
    ...
    collect_diagnostics, collect_from_expanded, collect_from_expanded_with_source_context,
    collect_with_model, expand_source, is_loud,
};
...
mod tests {
    ...
    fn fragment_context_is_an_explicit_coverage_fact() {
        ...
        let json = dump_json(&statements, &sem, &ctx, true, &DependencySection::default());
        let text = dump_text(&statements, &sem, &ctx, &DependencySection::default());
