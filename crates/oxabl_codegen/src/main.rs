...
const BUILTINS_RS_PATH: &str = "crates/oxabl_lexer/src/builtins.rs";
const FORMATTER_SPELLING_PATH: &str = "crates/oxabl_formatter/src/keyword_spelling.rs";
...
/// Collect the names of every built-in function from the JSON index.
///
/// This walks the raw entries and keeps **every** name whose title ends in
/// "` function`", independent of [`parse_json`]'s collapsed one-type-per-name
/// map. That distinction matters: many built-in functions share a name with a
/// statement or attribute (`ENTRY`, `LENGTH`, `AVAILABLE`, `NUM-ENTRIES`, ...),
/// and the collapsed map would drop the function role for those. Names are
/// returned uppercased.
pub fn parse_builtin_functions(path: &Path) -> Vec<String> {
    let content = fs::read_to_string(path).expect("Failed to read JSON file");
    let entries: Vec<JsonEntry> = serde_json::from_str(&content).expect("Failed to parse JSON");

    let mut names: HashSet<String> = HashSet::new();

    fn walk(entries: &[JsonEntry], names: &mut HashSet<String>) {
        for entry in entries {
            if let Some(name) = entry.title.trim().strip_suffix(" function") {
                let name = name.trim();
                // ABL identifiers never contain whitespace, so a multi-word
                // stem (e.g. "DEFINED preprocessor function", "NOT ENTERED
                // function") is a documentation phrase, not a callable name —
                // skip it rather than admit an unmatchable junk entry.
                if !name.is_empty() && !name.contains(char::is_whitespace) {
                    names.insert(name.to_uppercase());
                }
            }
            walk(&entry.child_entries, names);
        }
    }

    walk(&entries, &mut names);
...
/// Generate the `builtins.rs` file with the built-in ABL function registry.
///
/// Unlike the other generators this is driven directly off the JSON keyword
/// index (`json_info`) rather than the reserved-keyword list, because most
/// built-in functions (`LENGTH`, `SUBSTRING`, `ROUND`, `STRING`, ...) are **not**
...
pub fn generate_builtins_rs(function_names: &[String], keywords: &[Keyword]) -> String {
    ...
    output.push_str(&format!(
        "/// Sorted, ASCII-lowercased names of all built-in ABL functions ({} entries).\n",
        names.len()
    ));
    output.push_str("pub static BUILTIN_FUNCTIONS: &[&str] = &[\n");
    for name in &names {
    ...
    output.push_str(
        "/// Returns `true` if `name` is a built-in ABL function.\n\
         ...
         \x20   BUILTIN_FUNCTIONS.binary_search(&name).is_ok()\n\
         }\n",
    );
...
/// Generate `keyword_spelling.rs` for `oxabl_formatter`: a `Kind → canonical
/// full-form spelling` map for the keyword-abbreviation transform (U5).
///
/// Restricted to keywords that actually carry a `min_abbreviation` — the only
/// ones expansion applies to. This deliberately excludes punctuation/operator
/// aliases (which have no abbreviation), so a symbol `Kind` can never be
/// "expanded" into a word.
pub fn generate_formatter_spelling_rs(keywords: &[Keyword]) -> String {
    let mut out = String::new();
    out.push_str("// Generated by oxabl_codegen - DO NOT EDIT\n\n");
    out.push_str(
        "//! Canonical (full-form) spelling of each abbreviable keyword `Kind`, used by\n",
    );
    out.push_str("//! the keyword-abbreviation transform (U5) to expand an abbreviation to its\n");
    out.push_str("//! full ABL form. Generated from the same keyword source as the lexer's\n");
    out.push_str("//! `Kind` enum, so the two stay in lock-step (the plan's \"reuse the lexer's\n");
    out.push_str("//! keyword knowledge\" path).\n\n");
    out.push_str("use oxabl_lexer::Kind;\n\n");
    out.push_str("/// The canonical uppercase full-form spelling for an abbreviable keyword\n");
    out.push_str("/// `Kind`, or `None` for any kind that has no abbreviated form (punctuation,\n");
    out.push_str("/// operators, literals, identifiers, and preprocessor/include refs).\n");
    out.push_str("pub(crate) fn canonical_keyword(kind: Kind) -> Option<&'static str> {\n");
    out.push_str("    match kind {\n");
    let mut seen: HashSet<String> = HashSet::new();
    for kw in keywords {
        if kw.min_abbreviation.is_none() {
            continue;
        }
        // Single-word, purely-alphabetic keywords only. Expanding such an
        // abbreviation to its full form re-lexes to the same single keyword
        // `Kind`, so it can never change token structure. Multi-word (space or
        // hyphen) and `&`-prefixed keywords are deliberately excluded — their
        // stored canonical spelling is not a safe drop-in for the abbreviated
        // token, and preprocessor refs must never be rewritten (the guard
        // compares them by source text).
        if !kw.name.chars().all(|c| c.is_ascii_alphabetic()) {
            continue;
        }
        let variant = kw.to_variant_name();
        ...
        let canonical = kw.name.to_uppercase();
        out.push_str(&format!(
            "        Kind::{variant} => Some(\"{canonical}\"),\n"
        ));
    }
    out.push_str("        _ => None,\n");
    out.push_str("    }\n}\n");
...
fn main() {
    ...
    match command {
        ...
        "formatter-spelling" => {
            let content = generate_formatter_spelling_rs(&keywords);
