...
Build a correctness-first, blazingly-fast Rust foundation (lexer, parser, semantic analysis) and layer developer tooling on top, shipped as a single binary and delivered editor-first (LSP → VS Code). We win on three commitments competitors can't easily match: fast enough for per-keystroke interactivity, safe enough to never mangle code, and one shared pipeline behind every client — the client is never a variable in the answer. The first two decide whether developers actually leave the tooling on; the third means results differ only when the environment differs (a workspace, includes, a schema), never because you ran the editor instead of the CLI — so trust earned in one client carries to all of them.
...
The `oxabl` umbrella crate is now a curated, single-dependency public API: named modules per layer plus a few top-level conveniences (`parse`, `analyze`, `format_source`, `render_diagnostics`, `Diagnostic`), with internal helpers unreachable by construction and serde-serializable diagnostics behind an opt-in feature. The CLI, LSP, VS Code extension, and browser WASM demo consume this one surface instead of reaching into sub-crates or hand-rolling the tokenize→parse→analyze/format pipeline. The WASM crate is deliberately only a transport adapter: it maps shared diagnostics and formatter results to a small JSON wire shape, while unsupported project capabilities (includes and schema-backed checks in the first slice) stay explicitly unavailable rather than being reimplemented differently. Serving a browser client also forced the surface to earn its portability: native-only CLI dependencies now sit behind an opt-in `cli` feature, so the umbrella crate compiles for `wasm32` as a library, and CI builds that target on every push. **The run is now shared too, and the third commitment is a test rather than an aspiration.** `oxabl_pipeline` owns config resolution, both runs, and one result model; the CLI, the language server, and the browser are renderers of it. `oxabl.toml` is read once per resolution instead of per surface, and non-fatal config problems travel as data so a client chooses whether to show them rather than accidentally deciding they don't exist. `oxabl check` now enters that shared run — it is the lint-and-format gate, reporting findings and formatting drift in two channels, where before it never entered the shared analysis at all.

What made this worth doing was not deleting duplicated mapping code. The clients' final hops genuinely differ and still do: the editor's rope is the only correct position oracle under a negotiated encoding, so pipeline results stay byte-span-only and each client keeps its own last step. The value was that the layer *above* those hops was not merely duplicated but divergent — three config resolutions that disagreed, two file walkers that disagreed on both extensions and case, and a flagship command that ran a different pipeline from the editor while a source comment claimed otherwise. Those were wrong answers, not redundant code.

A cross-client parity suite now asserts that one source yields identical codes, severities, byte spans, and sources through all four entry points, and that a less-capable client reports an *unavailable capability* rather than a different answer. It earned itself immediately: two default severity tables meant the playground showed an error where the CLI showed a warning for the same code in the same empty environment. That is precisely the failure the commitment forbids, it had shipped unnoticed, and no amount of careful reading had caught it.

The reshaping also settled what the CLI *presents*. The visible surface is `check`, `format`, `lsp`, and `schema` — the four commands a developer using the tooling has a reason to run. The two project instruments, the parse-conformance walk that measures our parser against a corpus and the `analyze` semantic-model dump, stay in the shipped binary but hidden from `--help` and documented in the README: measuring our own parser is not a developer's question about their source. Their `--json` shapes remain explicitly unstable pre-1.0.
_Why it serves the approach:_ "Single binary, editor-first, consistent everywhere" only holds if there is one shared surface *and one shared run* the clients agree on — this track owns both.

### Browser try-it-out
The playground is live. The first browser slice compiles the shared single-file analysis and safe formatter pipelines to WebAssembly and embeds them in the Oxabl website: paste ABL, get the same lint diagnostics and the same never-mangle formatter output the CLI and editor produce. It runs entirely client-side, loads lazily, and is built as a versioned release artifact by the Oxabl repository (`oxabl-wasm-web.tar.gz`, with a wasm build gate in CI); the Railway-hosted static website only serves that artifact and the UI around it. Includes, workspace resolution, `oxabl.toml`, and schema upload are intentionally not simulated in the MVP — an unsupported project capability stays visibly unavailable rather than getting a second, divergent browser implementation.
...
- Simulating project context in the browser — includes, PROPATH resolution, `oxabl.toml`, and schema upload stay out of the playground; multi-file truth is what the CLI and editor are for.
