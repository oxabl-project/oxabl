...
The `oxabl` umbrella crate is now a curated, single-dependency public API: named modules per layer plus a few top-level conveniences (`parse`, `analyze`, `format_source`, `render_diagnostics`, `Diagnostic`), with internal helpers unreachable by construction and serde-serializable diagnostics behind an opt-in feature. The CLI, LSP, VS Code extension, and browser WASM demo consume this one surface instead of reaching into sub-crates or hand-rolling the tokenize→parse→analyze/format pipeline. The WASM crate is deliberately only a transport adapter: it maps shared diagnostics and formatter results to a small JSON wire shape, while unsupported project capabilities (includes and schema-backed checks in the first slice) stay explicitly unavailable rather than being reimplemented differently. Serving a browser client also forced the surface to earn its portability: native-only CLI dependencies now sit behind an opt-in `cli` feature, so the umbrella crate compiles for `wasm32` as a library, and CI builds that target on every push. The surface is curated, but the *run* is not: there is still no shared pipeline the clients drive. Config resolution (style, lint severities, include paths) is orchestrated separately per client, file-set orchestration exists only in the CLI, and the collected result is hand-mapped into a different shape four times over — LSP diagnostics, a browser wire struct, the analyze JSON envelope, and three unrelated format-outcome types. `oxabl check` is the sharpest instance: it never enters the shared analysis at all, running a parse-only conformance walk over its own result enum, so it reports neither lint nor format. Closing that gap — a lint pipeline and a format pipeline that own input resolution, file-set orchestration, and one result model every client merely renders — is what makes the third commitment true instead of aspirational; today's `check`/`analyze` and their `--json` are the transitional scaffolding it replaces. The reshaping also settles what the CLI *presents*: `check` becomes the ruff-shaped lint-and-format gate, and the two project instruments — the parse-conformance walk that measures the parser against a corpus, and the `analyze` semantic-model dump — stay in the shipped binary but hidden from `--help`. The visible surface is the four commands a developer has a reason to run; measuring our own parser is not one of them.
